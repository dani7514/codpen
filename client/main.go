...
func main() {
	...
	if err != nil {
		// If error has the prefix "codpen", then it was triggered by an event that wasn't an error, for example, exiting the editor.
		// It's a hacky solution since the UI returns an error only.
		if strings.HasPrefix(err.Error(), "codpen") {
