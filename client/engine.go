...
func handleTermboxEvent(ev termbox.Event, conn *websocket.Conn) error {
	...
	if ev.Type == termbox.EventKey {
		...
		case termbox.KeyEsc, termbox.KeyCtrlC:
			// Return an error with the prefix "codpen", so that it gets treated as an exit "event".
			return errors.New("codpen: exiting")

		// The default key for saving the editor's contents is Ctrl+S.
		case termbox.KeyCtrlS:
			// If no file name is specified, set filename to "codpen-content.txt"
			if fileName == "" {
				fileName = "codpen-content.txt"
			}
